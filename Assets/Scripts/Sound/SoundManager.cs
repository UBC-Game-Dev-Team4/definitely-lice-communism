...
namespace Sound
{
    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (Instance != null && privateStaticInstance != this) Debug.LogWarning("Multiple DialogueManagers In Scene! Keeping old singleton. " + Instance + ", " + this);
            if (Instance == null)
            {
                InitializeFields();
        ...
        public void ChangePitch(SoundOutputType type, float newPitch) => GetAudioMixerGroup(type).audioMixer.SetFloat(type + " Pitch", newPitch);
        /// <summary>
        ...
        public void SetVolume(SoundOutputType type, float newVolume) => GetAudioMixerGroup(type).audioMixer.SetFloat(type + " Volume", newVolume);
        /// <summary>
        /// Changes the volume of selected audio mixer to lowest possible volume.
        /// </summary>
        /// <param name="type">SoundAudioType of audio mixer.</param>
        /// <remarks>
        /// This does not keep a copy of the old volume. Any calls to <code>SetVolume(type,float)</code> will unmute it unless the newVolume == -80.
        /// </remarks>
        public void Mute(SoundOutputType type) => SetVolume(type, -80);
        ...
        private AudioMixerGroup GetAudioMixerGroup(SoundOutputType type) => MasterAudioMixer.FindMatchingGroups(type.GetAudioMixerString())[0];

        /// <summary>
        /// Plays a sound to a given sound output mixer.
        /// 
        /// Note that you can instead, in inspector, change the sound output audio mixer of the audio source, as this does the same thing.
        /// </summary>
    ...
    public static class SoundOutputTypeExtensions
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0066:Convert switch statement to expression", Justification = "Because Unity doesn't support it LOL")]
    ...
    {
        Master, Music, Dialogue, SFX, Ambient
    }
}